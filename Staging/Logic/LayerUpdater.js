...
},{"./lodash.custom.js":"node_modules/osmtogeojson/lodash.custom.js","@mapbox/geojson-rewind":"node_modules/@mapbox/geojson-rewind/index.js","osm-polygon-features":"node_modules/osm-polygon-features/index.js"}],"Logic/Osm/Overpass.ts":[function(require,module,exports) {
"use strict";
...
function () {
  ...
  Overpass.prototype.queryGeoJson = function (bounds, continuation, onFail) {
    var query = this.buildQuery("[bbox:" + bounds.south + "," + bounds.west + "," + bounds.north + "," + bounds.east + "]");
    ...
    jquery_1.default.getJSON(query, function (json, status) {
      if (status !== "success") {
...
},{"jquery":"node_modules/jquery/dist/jquery.js","osmtogeojson":"node_modules/osmtogeojson/index.js"}],"Logic/ElementStorage.ts":[function(require,module,exports) {
"use strict";
...
exports.State = State;
},{"./Utils":"Utils.ts","./Logic/ElementStorage":"Logic/ElementStorage.ts","./Logic/Osm/Changes":"Logic/Osm/Changes.ts","./Logic/Osm/OsmConnection":"Logic/Osm/OsmConnection.ts","./UI/i18n/Locale":"UI/i18n/Locale.ts","./UI/i18n/Translations":"UI/i18n/Translations.ts","./Logic/UIEventSource":"Logic/UIEventSource.ts","./Logic/Web/LocalStorageSource":"Logic/Web/LocalStorageSource.ts","./Logic/Web/QueryParameters":"Logic/Web/QueryParameters.ts"}],"Logic/LayerUpdater.ts":[function(require,module,exports) {
"use strict";
...
var Overpass_1 = require("./Osm/Overpass");
...
function () {
   ...
   */
  function LayerUpdater(state) {
    this.runningQuery = new UIEventSource_1.UIEventSource(false);
    this.retries = new UIEventSource_1.UIEventSource(0);
    /**
    ...
    this.previousBounds = new Map();
    this.state = state;
    var self = this;
    this.sufficentlyZoomed = State_1.State.state.locationControl.map(function (location) {
      if ((location === null || location === void 0 ? void 0 : location.zoom) === undefined) {
        return false;
      }

      var minzoom = Math.min.apply(Math, state.layoutToUse.data.layers.map(function (layer) {
        var _a;

        return (_a = layer.minzoom) !== null && _a !== void 0 ? _a : 18;
      }));
    ...
    state.locationControl.addCallback(function () {
      self.update(state);
    });
    state.layoutToUse.addCallback(function () {
      self.update(state);
    });
    self.update(state);
  }

  LayerUpdater.prototype.GetFilter = function (state) {
    var filters = [];
    state = state !== null && state !== void 0 ? state : State_1.State.state;

    for (var _i = 0, _a = state.layoutToUse.data.layers; _i < _a.length; _i++) {
      var layer = _a[_i];
      ...
      for (var z = layer.minzoom; z < 25 && !previouslyLoaded; z++) {
        var previousLoadedBounds = this.previousBounds.get(z);

        if (previousLoadedBounds === undefined) {
          continue;
        }

        for (var _b = 0, previousLoadedBounds_1 = previousLoadedBounds; _b < previousLoadedBounds_1.length; _b++) {
          var previousLoadedBound = previousLoadedBounds_1[_b];
  ...
  LayerUpdater.prototype.handleData = function (geojson) {
    ...
    function renderLayers(layers) {
      ...
      window.setTimeout(function () {
        var layer = layers[0];
        var rest = layers.slice(1, layers.length);
        geojson = layer.SetApplicableData(geojson);
        renderLayers(rest);
      }, 50);
    }

    renderLayers(State_1.State.state.filteredLayers.data);
  };

  LayerUpdater.prototype.handleFail = function (state, reason) {
    this.retries.data++;
    this.ForceRefresh();
  ...
  LayerUpdater.prototype.update = function (state) {
    var filter = this.GetFilter(state);
    ...
    var bounds = state.bm.map.getBounds();
    var diff = state.layoutToUse.data.widenFactor;
    var n = Math.min(90, bounds.getNorth() + diff);
    var e = Math.min(180, bounds.getEast() + diff);
    var s = Math.max(-90, bounds.getSouth() - diff);
    var w = Math.max(-180, bounds.getWest() - diff);
    var queryBounds = {
      north: n,
      east: e,
      south: s,
    ...
    var z = Math.floor(state.locationControl.data.zoom);
    this.runningQuery.setData(true);
    var self = this;
    var overpass = new Overpass_1.Overpass(filter);
    overpass.queryGeoJson(queryBounds, function (data) {
...
},{"./Tags":"Logic/Tags.ts","./UIEventSource":"Logic/UIEventSource.ts","./Osm/Overpass":"Logic/Osm/Overpass.ts","../State":"State.ts"}],"node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
