...
},{"./Translation":"UI/i18n/Translation.ts","../Base/FixedUiElement":"UI/Base/FixedUiElement.ts"}],"UI/Base/SubtleButton.ts":[function(require,module,exports) {
"use strict";
...
},{"../UIElement":"UI/UIElement.ts","../i18n/Translations":"UI/i18n/Translations.ts","./Combine":"UI/Base/Combine.ts","./FixedUiElement":"UI/Base/FixedUiElement.ts"}],"Logic/ElementStorage.ts":[function(require,module,exports) {
"use strict";
...
exports.State = State;
},{"./Utils":"Utils.ts","./Logic/ElementStorage":"Logic/ElementStorage.ts","./Logic/Osm/Changes":"Logic/Osm/Changes.ts","./Logic/Osm/OsmConnection":"Logic/Osm/OsmConnection.ts","./UI/i18n/Locale":"UI/i18n/Locale.ts","./UI/i18n/Translations":"UI/i18n/Translations.ts","./Logic/UIEventSource":"Logic/UIEventSource.ts","./Logic/Web/LocalStorageSource":"Logic/Web/LocalStorageSource.ts","./Logic/Web/QueryParameters":"Logic/Web/QueryParameters.ts"}],"UI/SimpleAddUI.ts":[function(require,module,exports) {
"use strict";
...
function (_super) {
  ...
  function SimpleAddUI() {
    var _this = _super.call(this, State_1.State.state.locationControl) || this;

    _this._confirmPreset = new UIEventSource_1.UIEventSource(undefined);
    _this.confirmButton = undefined;
    _this.goToInboxButton = new SubtleButton_1.SubtleButton("./assets/envelope.svg", Translations_1.default.t.general.goToInbox, {
    ...
    _this.ListenTo(State_1.State.state.layerUpdater.runningQuery);

    _this.ListenTo(_this._confirmPreset);

    _this.ListenTo(State_1.State.state.locationControl);

    _this._loginButton = Translations_1.default.t.general.add.pleaseLogin.Clone().onClick(function () {
      return State_1.State.state.osmConnection.AttemptLogin();
    });
    _this._addButtons = [];

    _this.SetStyle("font-size:large");

    var self = _this;

    var _loop_1 = function _loop_1(layer) {
      this_1.ListenTo(layer.isDisplayed);

      var _loop_2 = function _loop_2(preset) {
        var icon = "./assets/bug.svg";

        if (preset.icon !== undefined) {
          if (typeof preset.icon !== "string") {
            var tags = Utils_1.Utils.MergeTags(Tags_1.TagUtils.KVtoProperties(preset.tags), {
              id: "node/-1"
            });
        ...
        var csCount = State_1.State.state.osmConnection.userDetails.data.csCount;
        var tagInfo = "";

        if (csCount > State_1.State.userJourney.tagsVisibleAt) {
          tagInfo = preset.tags.map(function (t) {
            return t.asHumanString(false, true);
          }).join("&");
          tagInfo = "<br/><span class='subtle'>" + tagInfo + "</span>";
        }

        var button = new SubtleButton_1.SubtleButton(icon, new Combine_1.default(["<b>", preset.title, "</b>", preset.description !== undefined ? new Combine_1.default(["<br/>", preset.description]) : "", tagInfo])).onClick(function () {
          self.confirmButton = new SubtleButton_1.SubtleButton(icon, new Combine_1.default(["<b>", Translations_1.default.t.general.add.confirmButton.Subs({
            category: preset.title
          }), "</b><br/>", preset.description !== undefined ? preset.description : ""]));
        ...
        this_1._addButtons.push(button);
      };

      for (var _i = 0, _a = layer.layerDef.presets; _i < _a.length; _i++) {
        var preset = _a[_i];
    ...
    for (var _i = 0, _a = State_1.State.state.filteredLayers.data; _i < _a.length; _i++) {
      var layer = _a[_i];

      _loop_1(layer);
    }

    _this.cancelButton = new SubtleButton_1.SubtleButton("./assets/close.svg", Translations_1.default.t.general.cancel).onClick(function () {
      self._confirmPreset.setData(undefined);
    });
    _this.openLayerControl = new SubtleButton_1.SubtleButton("./assets/layers.svg", Translations_1.default.t.general.add.openLayerControl).onClick(function () {
      State_1.State.state.layerControlIsOpened.setData(true);
    });
    return _this;
  }

  SimpleAddUI.prototype.CreatePoint = function (tags, layerToAddTo) {
    return function () {
      var loc = State_1.State.state.bm.LastClickLocation.data;
      var feature = State_1.State.state.changes.createElement(tags, loc.lat, loc.lon);
      layerToAddTo.AddNewElement(feature);
      State_1.State.state.selectedElement.setData({
        feature: feature
      });
    };
  };
  ...
  SimpleAddUI.prototype.InnerRender = function () {
    var userDetails = State_1.State.state.osmConnection.userDetails;

    if (this._confirmPreset.data !== undefined) {
      if (!this._confirmPreset.data.layerToAddTo.isDisplayed.data) {
        return new Combine_1.default([Translations_1.default.t.general.add.layerNotEnabled.Subs({
          layer: this._confirmPreset.data.layerToAddTo.layerDef.name
        }).SetClass("alert"), this.openLayerControl, this.cancelButton]).Render();
      }

      var tagInfo = "";
      var csCount_1 = State_1.State.state.osmConnection.userDetails.data.csCount;

      if (csCount_1 > State_1.State.userJourney.tagsVisibleAt) {
        tagInfo = this._confirmPreset.data.tags.map(function (t) {
          return t.asHumanString(csCount_1 > State_1.State.userJourney.tagsVisibleAndWikiLinked, true);
        }).join("&");
        tagInfo = "<br/>More information about the preset: " + tagInfo;
      }

      return new Combine_1.default([Translations_1.default.t.general.add.confirmIntro.Subs({
        title: this._confirmPreset.data.name
      }), userDetails.data.dryRun ? "<span class='alert'>TESTING - changes won't be saved</span>" : "", this.confirmButton, this.cancelButton, tagInfo]).Render();
    }
    ...
    if (!userDetails.data.loggedIn) {
      return new Combine_1.default([header, this._loginButton]).Render();
    }

    if (userDetails.data.unreadMessages > 0 && userDetails.data.csCount < State_1.State.userJourney.addNewPointWithUnreadMessagesUnlock) {
      return new Combine_1.default([header, "<span class='alert'>", Translations_1.default.t.general.readYourMessages, "</span>", this.goToInboxButton]).Render();
    }

    if (userDetails.data.dryRun) {
      header = new Combine_1.default([header, "<span class='alert'>", "Test mode - changes won't be saved", "</span>"]);
    }

    if (userDetails.data.csCount < State_1.State.userJourney.addNewPointsUnlock) {
      return new Combine_1.default([header, "<span class='alert'>", Translations_1.default.t.general.fewChangesBefore, "</span>"]).Render();
    }

    if (State_1.State.state.locationControl.data.zoom < State_1.State.userJourney.minZoomLevelToAddNewPoints) {
      return new Combine_1.default([header, Translations_1.default.t.general.add.zoomInFurther]).Render();
    }

    if (State_1.State.state.layerUpdater.runningQuery.data) {
      return new Combine_1.default([header, Translations_1.default.t.general.add.stillLoading]).Render();
    }

    return header.Render() + new Combine_1.default(this._addButtons).SetClass("add-popup-all-buttons").Render();
  };
...
},{"./UIElement":"UI/UIElement.ts","../Logic/Tags":"Logic/Tags.ts","./i18n/Translations":"UI/i18n/Translations.ts","./Base/Combine":"UI/Base/Combine.ts","./Base/SubtleButton":"UI/Base/SubtleButton.ts","./i18n/Locale":"UI/i18n/Locale.ts","../State":"State.ts","../Logic/UIEventSource":"Logic/UIEventSource.ts","../Utils":"Utils.ts"}],"node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
