...
exports.Utils = Utils;
},{"./UI/UIElement":"UI/UIElement.ts","jquery":"node_modules/jquery/dist/jquery.js","./UI/Base/FixedUiElement":"UI/Base/FixedUiElement.ts"}],"UI/Input/OpeningHoursTable.ts":[function(require,module,exports) {
"use strict";
...
/** @class */
function (_super) {
  __extends(OpeningHoursTable, _super);

  function OpeningHoursTable(source) {
    if (source === void 0) {
      source = undefined;
    }
    ...
    _this.source = source !== null && source !== void 0 ? source : new UIEventSource_1.UIEventSource(undefined);
    _this.IsSelected = new UIEventSource_1.UIEventSource(false);

    _this.SetStyle("width:100%;height:100%;display:block;");

    return _this;
  }

  OpeningHoursTable.prototype.InnerRender = function () {
    var rows = "";
    ...
    for (var h = 0; h < 24; h++) {
      ...
      rows += "<tr><td rowspan=\"2\" class=\"oh-left-col oh-timecell-full\">" + hs + ":00</td>" + Utils_1.Utils.Times('<td class="oh-timecell oh-timecell-full"></td>', 7) + '</tr><tr>' + // Utils.Times('<td class="oh-timecell"></td>', 7) +
      // '</tr><tr>' +
      Utils_1.Utils.Times('<td class="oh-timecell oh-timecell-half"></td>', 7) + //  '</tr><tr>' +
      //  Utils.Times('<td class="oh-timecell"></td>', 7) +
      '</tr>';
    }

    var days = OpeningHoursTable.days.join("</th><th>");
    return "<table id=\"oh-table-" + this.id + "\" class=\"oh-table\"><tr><th></th><th>" + days + "</tr>" + rows + "</table>";
  };

  OpeningHoursTable.prototype.InnerUpdate = function () {
    var self = this;
    var table = document.getElementById("oh-table-" + this.id);
    ...
    function h(timeSegment) {
      return Math.floor(timeSegment / 4);
    }

    function m(timeSegment) {
      return timeSegment % 2 * 30;
    }

    function startSelection(i, j, cell) {
      mouseIsDown = true;
      selectionStart = [i, j];
      selectionEnd = [i, j];
    ...
    function endSelection() {
      ...
      mouseIsDown = false;
      var dStart = Math.min(selectionStart[1], selectionEnd[1]);
      var dEnd = Math.max(selectionStart[1], selectionEnd[1]);
      var timeStart = Math.min(selectionStart[0], selectionEnd[0]) - 1;
      var timeEnd = Math.max(selectionStart[0], selectionEnd[0]) - 1;
      var oh = {
        weekdayStart: dStart,
        weekdayEnd: dEnd,
    ...
    function selectAllBetween(iEnd, jEnd) {
      var iStart = selectionStart[0];
      var jStart = selectionStart[1];
    ...
    var _loop_1 = function _loop_1(i) {
      ...
      var _loop_2 = function _loop_2(j) {
        ...
        cell.ontouchmove = function (ev) {
          ...
          for (var k in ev.targetTouches) {
            var touch = ev.targetTouches[k];
            var elUnderTouch = document.elementFromPoint(touch.screenX, touch.screenY); // @ts-ignore

            var f = elUnderTouch.onmouseenter;
        ...
        cell.ontouchend = function (ev) {
          ev.preventDefault();

          for (var k in ev.targetTouches) {
            var touch = ev.targetTouches[k];
            var elUnderTouch = document.elementFromPoint(touch.pageX, touch.pageY); // @ts-ignore

            var f = elUnderTouch.onmouseup;
  ...
  OpeningHoursTable.prototype.IsValid = function (t) {
    return true;
  };

  OpeningHoursTable.prototype.GetValue = function () {
    return this.source;
  };

  OpeningHoursTable.days = ["Maan", "Din", "Woe", "Don", "Vrij", "Zat", "Zon"];
  return OpeningHoursTable;
}(InputElement_1.InputElement);

exports.default = OpeningHoursTable;
},{"./InputElement":"UI/Input/InputElement.ts","../../Logic/UIEventSource":"Logic/UIEventSource.ts","../../Utils":"Utils.ts"}],"test.ts":[function(require,module,exports) {
"use strict";
...
var OpeningHoursTable_1 = __importDefault(require("./UI/Input/OpeningHoursTable"));

var oh = new OpeningHoursTable_1.default();
oh.SetStyle("height:100vh;display:block;").AttachTo('maindiv');
oh.GetValue().addCallback(function (data) {
  return console.log(data);
});
new VariableUIElement_1.VariableUiElement(oh.GetValue().map(function (oh) {
  if (oh === undefined) {
    return "<no value selected>";
  }

  return oh.weekdayStart + " " + oh.startHour + ":" + oh.startMinutes + " --> " + oh.weekdayEnd + " " + oh.endHour + ":" + oh.endMinutes;
})).AttachTo("extradiv"); //*/
...
},{"./UI/Base/VariableUIElement":"UI/Base/VariableUIElement.ts","./UI/Input/OpeningHoursTable":"UI/Input/OpeningHoursTable.ts"}],"node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
